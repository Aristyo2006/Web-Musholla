...
<body class="bg-slate-50 dark:bg-zinc-950 text-zinc-900 dark:text-emerald-50 selection:bg-emerald-500 selection:text-white transition-colors duration-500 overflow-x-hidden">

    @include('partials.navbar', ['transparentTheme' => true])

    <!-- Hero Section -->
    <section id="hero" class="relative min-h-[110vh] flex items-center justify-center overflow-hidden emerald-gradient pt-32 pb-20">
        <div class="absolute inset-0 z-0">
            ...
            <div class="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-slate-50 dark:to-zinc-950 transition-colors duration-500"></div>
        </div>
        <div class="relative z-10 max-w-5xl px-6 text-center">
            ...
            <div class="flex flex-col items-center gap-4 mt-8 animate-fade-in-up" style="animation-delay: 0.3s">
                ...
                <p class="text-[10px] font-bold text-emerald-50/40 uppercase tracking-widest italic">
                    Donasi Terbaru: <span class="text-amber-400/80">{{ $latestCampaign->title ?? 'Program Musholla' }}</span>
                </p>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-6 sm:px-6 lg:px-8 relative z-10 w-full">
            <div class="w-full lg:w-1/2 pr-0 lg:pr-16 animate-fade-in-up">
                <div
                    ...
                    Sejarah & Profil</div>
                <h2 class="text-4xl md:text-7xl font-black text-zinc-900 dark:text-white tracking-tighter mb-10 leading-[1.05]">
                    {{ $settings['about_title'] ?? 'Tentang Musholla Al-Kautsar' }}
                </h2>
                <div class="text-zinc-600 dark:text-emerald-100/40 text-xl md:text-2xl leading-relaxed space-y-8 lg:max-w-xl font-medium italic">
    ...
    <section id="carousel" class="py-24 md:py-40 bg-slate-50 dark:bg-zinc-950 overflow-hidden relative transition-colors duration-500">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16 md:mb-24">
                <h2 class="text-4xl md:text-6xl font-black text-zinc-900 dark:text-white mb-6 md:mb-8 tracking-tighter leadgin-tight">Pojok <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-emerald-400 dark:from-emerald-400 dark:to-amber-300">Dokumentasi</span></h2>
                <p class="text-zinc-500 dark:text-emerald-100/30 max-w-2xl mx-auto text-lg md:text-xl font-medium italic">"Setiap sudut pembangunan adalah saksi bisu perjuangan kita bersama untuk rumah Allah."</p>
            </div>
            ...
            <div class="relative h-[500px] md:h-[700px] group">
                ...
                <div
                    ...
                    @forelse($featuredGalleries as $key => $gallery)
                        <div class="carousel-item {{ $key === 0 ? 'active' : '' }} absolute inset-0">
                            <img src="{{ Storage::url($gallery->image) }}" alt="{{ $gallery->title ?? ($gallery->campaign->title ?? 'Dokumentasi') }}"
                                class="w-full h-full object-cover">
                            <div
                                ...
                                <div class="max-w-[70%] md:max-w-none mx-auto md:mx-0">
                                    ...
                                    <div class="flex flex-wrap items-center gap-2 mb-3 md:mb-4">
                                        @if($gallery->campaign_id)
                                            <div class="bg-emerald-500 text-white text-[8px] md:text-[10px] font-black px-3 py-1 rounded-full w-fit uppercase tracking-widest flex items-center gap-1.5 shadow-lg">
                                                <span class="w-1 h-1 bg-white rounded-full animate-pulse"></span>
                                                DONASI
                                            </div>
                                        @endif
                                        @if($gallery->badge)
                                            <div class="bg-amber-500 text-emerald-950 text-[8px] md:text-[10px] font-black px-3 py-1 rounded-full w-fit uppercase tracking-widest shadow-lg">
                                    ...
                                    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6">
                                        <div class="max-w-3xl">
                                            <h3 class="text-white text-base md:text-5xl font-black mb-1 md:mb-3 tracking-tighter leading-tight drop-shadow-2xl">
                                                {{ $gallery->title ?? ($gallery->campaign->title ?? 'Musholla Al-Kautsar') }}
                                            </h3>
                                            <p class="text-emerald-50/80 text-[10px] md:text-xl font-medium line-clamp-2 md:line-clamp-3 italic max-w-2xl">
                                        ...
                                        <div class="flex-shrink-0 pt-2 md:pt-0">
                                            <a href="{{ route('donasi.index', $gallery->campaign->slug) }}" 
                                               class="inline-flex items-center gap-2 px-4 py-2.5 md:px-6 md:py-4 bg-emerald-500 hover:bg-amber-500 text-white hover:text-emerald-950 font-black text-[9px] md:text-sm uppercase tracking-widest rounded-xl md:rounded-2xl transition-all duration-500 shadow-xl group/btn">
                                                Donasi Sekarang
                                                <svg class="w-3 h-3 md:w-4 md:h-4 group-hover/btn:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button onclick="prevSlide()"
                    ...
                    <svg class="w-5 h-5 md:w-8 md:h-8 relative z-10 group-hover:scale-110 transition-transform" fill="none"
    ...
    <section id="articles" class="py-24 md:py-48 bg-white dark:bg-zinc-950 transition-colors duration-500 relative">
        ...
        <div class="absolute inset-0 overflow-hidden pointer-events-none opacity-50 dark:opacity-0 transition-opacity duration-500">
        ...
        <div class="max-w-7xl mx-auto px-6 sm:px-6 lg:px-8 relative z-10">
            <div
                ...
                <div>
                    <h2 class="text-4xl md:text-7xl font-black text-zinc-900 dark:text-white mb-6 md:mb-10 leading-tight tracking-tight">
                        Kabar <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-emerald-400 dark:from-emerald-400 dark:to-amber-300 italic px-2">Donatur</span></h2>
                    <p class="text-zinc-500 dark:text-emerald-100/30 text-xl font-medium italic">Ikuti setiap jengkal kemajuan dan kebahagiaan dari Musholla Al-Kautsar.
                    </p>
                </div>
                <a href="{{ route('articles.index') }}"
                    ...
                    Arsip Kabar
                    <div class="w-12 h-12 rounded-2xl bg-emerald-500/10 dark:bg-white/5 border border-emerald-500/20 dark:border-white/10 flex items-center justify-center group-hover:scale-110 group-hover:bg-emerald-600 dark:group-hover:bg-emerald-500 group-hover:text-white dark:group-hover:text-emerald-950 transition-all duration-500 shadow-xl shadow-emerald-500/5">
            ...
            <div class="mt-24 md:mt-32">
                <div
                    ...
                    <div class="relative z-10 max-w-2xl px-2 md:px-0">
                        ...
                        <p class="text-emerald-50/70 dark:text-emerald-100/70 text-xl md:text-2xl font-medium italic leading-relaxed">"Sedekah Anda adalah tabungan terbaik untuk masa depan dunia dan akhirat."</p>
                    </div>
        ...
        <button onclick="window.scrollTo({top: 0, behavior: 'smooth'})" id="btn-back-to-top"
           class="relative flex items-center justify-center w-14 h-14 md:w-16 md:h-16 transition-all duration-500 group active:scale-90 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
            <div class="glass-btn-bg absolute inset-0 rounded-full pointer-events-none transition-all duration-500 z-0" 
                 style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(255,255,255,0.2); transition: all 0.5s cubic-bezier(0.4, 0, 0.2, 1); will-change: backdrop-filter, background-color;"></div>
            <svg class="w-5 h-5 md:w-6 md:h-6 text-white relative z-10 transition-transform duration-500 group-hover:-translate-y-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
