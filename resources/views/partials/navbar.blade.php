...
<header
    class="fixed w-full z-40 transition-all duration-500 {{ !$transparentTheme ? 'bg-emerald-950/90 backdrop-blur-md shadow-lg border-b border-emerald-900/50' : 'bg-transparent border-b border-transparent shadow-none' }}"
    id="main-header">
    <nav class="w-full relative transition-all duration-500 {{ !$transparentTheme ? 'py-1' : '' }}" id="nav-container">
        @if($transparentTheme)
            <div id="navbar-blur-bg"
                class="absolute inset-0 transition-all duration-500 opacity-0 bg-emerald-950/80 backdrop-blur-md pointer-events-none">
            </div>
        @endif
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center relative z-10"
            ...
            <div class="flex items-center gap-3">
                <a href="{{ url('/') }}" class="flex items-center gap-3 hover:scale-105 transition-all">
                    ...
                    <div class="flex flex-col leading-tight">
                        ...
                        <span class="text-[10px] text-emerald-300 font-bold tracking-[0.2em] uppercase"></span>
                    </div>
                </a>
            </div>
            ...
            <div class="hidden lg:flex items-center gap-8 text-white font-medium">
                    ...
                    @auth
                        @if(Auth::user()->isAdmin())
                            <a href="{{ route('dashboard') }}"
                                class="px-5 py-2 {{ $transparentTheme ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-emerald-800 hover:bg-emerald-700 border border-emerald-700' }} rounded-full transition shadow-lg shadow-emerald-900/20 text-sm">Dashboard
                                Admin</a>
                        @endif
                    @else
                ...
                <a href="{{ route('campaigns.index') }}"
                    class="px-6 py-2 rounded-full bg-amber-500 text-emerald-950 font-bold shadow-lg shadow-amber-500/20 hover:scale-105 hover:bg-amber-400 transition-all text-sm">
                    Donasi Sekarang
                </a>
            </div>
        ...
        <!-- Mobile Menu Drawer -->
        <div id="mobile-menu"
            class="fixed inset-0 bg-emerald-950/95 backdrop-blur-xl z-[45] flex flex-col pt-32 px-8 gap-8 text-white transition-all duration-500 translate-x-full lg:hidden overflow-y-auto pb-12">
            <a href="{{ url('/#hero') }}" class="text-3xl font-black hover:text-amber-400 transition"
                onclick="toggleMenu()">Beranda</a>
            <a href="{{ route('campaigns.index') }}" class="text-3xl font-black hover:text-amber-400 transition"
                onclick="toggleMenu()">Program Donasi</a>
            <a href="{{ route('galleries.index') }}" class="text-3xl font-black hover:text-amber-400 transition"
                onclick="toggleMenu()">Galeri</a>
            <a href="{{ url('/#articles') }}" class="text-3xl font-black hover:text-amber-400 transition"
            ...
            @if (Route::has('login'))
                @auth
                    @if(Auth::user()->isAdmin())
                        <a href="{{ route('dashboard') }}" class="text-xl font-bold text-emerald-400"
                ...
                @else
                    <a href="{{ route('login') }}" class="text-3xl font-black hover:text-amber-400 transition"
                        onclick="toggleMenu()">Masuk</a>
                @endauth
            @endif

            <a href="{{ route('campaigns.index') }}"
                class="mt-4 px-8 py-5 bg-amber-500 text-emerald-950 text-center font-black rounded-3xl shadow-2xl shadow-amber-500/20 active:scale-95 transition-all text-xl"
                onclick="toggleMenu()">
                Donasi Sekarang
            </a>
        </div>
